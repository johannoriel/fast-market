...
def test_paths_follow_xdg(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setenv("FASTMARKET_PROFILE", "default")

    assert (
        get_fastmarket_dir()
        == data_home / "fast-market" / "profiles" / "default" / "data"
    )
    assert (
        get_tool_config("corpus")
        == config_home / "fast-market" / "profiles" / "default" / "corpus" / "config.yaml"
    )

    corpus_dir = get_tool_data_dir("corpus")
    marketing_dir = get_tool_data_dir("marketing")
    assert corpus_dir == data_home / "fast-market" / "profiles" / "default" / "corpus"
    assert marketing_dir == data_home / "fast-market" / "profiles" / "default" / "marketing"
    assert corpus_dir.exists()
    assert marketing_dir.exists()

    cache_dir = get_tool_cache_dir("corpus")
    assert cache_dir == cache_home / "fast-market" / "profiles" / "default" / "corpus"
...
def test_load_config_from_override_dir(monkeypatch, tmp_path: Path):
    ...
    # corpus requires the llm + youtube common subconfigs (registered by
    # cli/main.py); provide them so this test is order-independent.
    common_dir = home / ".config" / "fast-market" / "profiles" / "default" / "common"
    for subconfig in ("llm", "youtube"):
        sub_dir = common_dir / subconfig
        sub_dir.mkdir(parents=True)
        (sub_dir / "config.yaml").write_text("{}", encoding="utf-8")
...
def test_paths_expand_tilde_in_xdg_env(monkeypatch, tmp_path: Path):
    ...
    assert (
        get_fastmarket_dir()
        == home / ".custom_data" / "fast-market" / "profiles" / "default" / "data"
    )
    assert (
        get_tool_cache_dir("corpus")
        == home / ".custom_cache" / "fast-market" / "profiles" / "default" / "corpus"
    )
...
def test_sqlite_store_default_path_is_tool_data(monkeypatch, tmp_path: Path):
    ...
    assert "fast-market/profiles/default/corpus" in str(db_path)
...
def test_youtube_transport_expands_tilde_client_secret_path(
    ...
    oauth = YouTubeOAuth(client_secret_path="~/secrets/client_secret.json")
    token_path = Path(oauth.client_secret_path).expanduser().parent / "token.json"
