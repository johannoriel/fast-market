# Test script to check if we can retrieve transcript from member-only video bndvzwbgNSE
...
def test_transcript_via_api_captions():
    ...
    try:
        ...
        # List captions
        response = (
            client.youtube.captions().list(part="snippet", videoId=VIDEO_ID).execute()
        )
        ...
        if response.get("items"):
            ...
            for item in response["items"]:
                ...
                print(
                    f"  - {snippet.get('name', 'No name')} ({snippet.get('language', 'N/A')})"
                )
            ...
            caption = (
                client.youtube.captions().download(id=caption_id, tfmt="srt").execute()
            )
...
def test_plugin_fetch():
    ...
    try:
        ...
        item_meta = ItemMeta(
            ...
            metadata={
                "id": VIDEO_ID,
                "title": "Member-only test video",
                "description": "",
                "published_at": "",
                "url": VIDEO_URL,
                "duration_seconds": 0,
                "privacy_status": "members",  # Test with members status
            },
        )
