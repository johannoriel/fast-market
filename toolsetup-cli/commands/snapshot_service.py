...
def do_snapshot(
    ...
    click.echo(f"{source_type.capitalize()} snapped. {snap_count} item(s) copied to: {target_dir}")
...
def do_restore(
    ...
    if not target_dir.exists():
        ...
        _save_state(snapshot_root, source_type, {"snapped": False, "targets": [], "sentinel": None})
        return 0
    ...
    # Reset state
    _save_state(snapshot_root, source_type, {"snapped": False, "targets": [], "sentinel": None})
    click.echo(f"{source_type.capitalize()} restored. {restore_count} item(s) restored.")
    return restore_count
...
def list_snapshots(
    ...
    snapshots = sorted(
        [d for d in snapshot_dir.iterdir() if d.is_dir() and d.name.startswith(sentinel_prefix)],
    ...
    for snap in snapshots:
        ...
        click.echo(f"  {snap.name} - {mtime} - {item_count} item(s){marker}")
...
def do_rollback(
    ...
    if not state.get("snapped"):
        click.echo(f"{source_type.capitalize()} is not currently snapped. Nothing to rollback.")
        return
    ...
    if re_snap:
        ...
        if source_path and source_path.exists():
            do_snapshot(source_type, source_path, flat_only=flat_only, sentinel_prefix=source_type)
