...
def _get_config_path() -> Path:
    ...
    return get_tool_config_path("monitor")
...
def _sync_config_logic(
    ...
    for src in config.get("sources", []):
        ...
        if src_id in existing_sources:
            ...
            slowdown_val = src.get("slowdown")
            if isinstance(slowdown_val, str):
                from core.time_scheduler import parse_interval

                try:
                    slowdown_val = int(parse_interval(slowdown_val).total_seconds())
