...
class TestSourcePluginCooldown:
    ...
    def test_no_default_slowdown(self):
        plugin = YouTubePlugin({}, {"origin": "UCabcdef123456ghijklmnop"})
        assert plugin.slowdown is None

    def test_no_default_slowdown_rss(self):
        plugin = RSSPlugin({}, {"origin": "https://example.com/feed.xml"})
        assert plugin.slowdown is None

    def test_no_default_slowdown_yt_search(self):
    ...
    def test_should_fetch_with_last_check_none_string(self):
        plugin = YouTubePlugin({}, {"origin": "UCabcdef123456ghijklmnop", "last_check": None})
        assert plugin._should_fetch() is True
...
class TestYouTubePlugin:
    ...
    def test_validate_identifier_channel_url(self):
        plugin = YouTubePlugin(
            {}, {"origin": "https://www.youtube.com/channel/UCabcdef123456ghijklmnop"}
        )
        assert (
...
class TestYouTubeSearchPlugin:
    ...
    def test_validate_identifier_advanced(self):
        plugin = YouTubeSearchPlugin({}, {"origin": "AI tutorial -shorts | ML"})
        assert plugin.validate_identifier("AI tutorial -shorts | ML") is True

    def test_validate_identifier_phrase(self):
        plugin = YouTubeSearchPlugin({}, {"origin": "machine learning"})
