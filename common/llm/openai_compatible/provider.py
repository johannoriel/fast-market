...
class OpenAICompatibleProvider(LazyLLMProvider):
    ...
    def _initialize(self):
        ...
        api_key = resolve_secret(provider_config, default_env="OPENAI_COMPATIBLE_API_KEY")
        key_required = bool(api_key_env) and str(api_key_env).upper() not in ("", "NONE")
        if key_required and not api_key:
            logger.warning(
                "openai_compatible_provider_not_initialized",
                reason="no inline api_key and api_key_env/OPENAI_COMPATIBLE_API_KEY not set",
            )
            self._provider = None
            return
