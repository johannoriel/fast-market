...
def edit_skill_agent_config() -> bool:
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".yaml", prefix="fastmarket-skill-", delete=False
    ) as f:
    ...
    try:
        ...
        if "agent" in new_config:
            save_skill_agent_config(new_config["agent"])
        else:
            save_skill_agent_config(new_config)
...
def _validate_full_config(config: dict) -> list[str]:
    ...
    if "agent" in config:
        ...
        if not isinstance(agent, dict):
            ...
            agent_errors = _validate_agent_config(agent)
...
def _validate_agent_config(agent: dict) -> list[str]:
    errors = []
    ...
    if "default_timeout" in agent:
        if (
            not isinstance(agent["default_timeout"], int)
            or agent["default_timeout"] < 1
        ):
            errors.append("default_timeout must be a positive integer")

    if "fastmarket_tools" in agent:
        ft = agent["fastmarket_tools"]
    ...
    if "system_commands" in agent:
        if not isinstance(agent["system_commands"], list):
            errors.append("system_commands must be a list")
        elif not all(isinstance(c, str) for c in agent["system_commands"]):
            errors.append("system_commands must contain only strings")

    if "agent_prompt" in agent:
        ap = agent["agent_prompt"]
