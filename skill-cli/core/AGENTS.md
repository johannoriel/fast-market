...
where `run_root = {workdir_root}/{workdir_prefix}{uuid}/` (or `{workdir}/skill_run_{uuid}/` if `workdir_root` is not configured, using `workdir_prefix` from common config or defaulting to `skill_run_`).
