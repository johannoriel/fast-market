...
def _long_meta_path(source: str) -> Path:
    p = Path(source)
    return p.parent / f"{p.stem}-long-meta.json"
...
def _save_meta(job) -> None:
    ...
    try:
        with open(_long_meta_path(job.source), "w", encoding="utf-8") as f:
...
async def _run_capture(step, *cmd: str):
    """Like ``_run`` but returns the full stdout text (for parsing JSON)."""
    from .state import set_active_proc, clear_active_proc
