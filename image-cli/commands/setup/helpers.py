...
def get_config_path() -> Path:
    """Return the image-agent config file path."""
    return get_tool_config("image")


def load_config(config_path: Path) -> dict:
    """Load config from file, return defaults if it does not exist."""
...
def save_config(config_path: Path, config: dict) -> None:
    ...
    config_path.write_text(dump_yaml(config, sort_keys=False), encoding="utf-8")


def reset_config(config_path: Path) -> None:
    ...
    click.echo("Use 'image setup show' to review the default settings.")


def require_supported(engine_name: str) -> str:
...
def get_engine_defaults(engine_name: str) -> dict:
    """Return default config for an engine."""
    return get_default_config().get("engines", {}).get(engine_name, {})


def list_engines(config: dict) -> None:
...
def add_engine(config_path: Path, config: dict, engine_name: str) -> None:
    """Add a new engine configuration."""
    try:
        normalized = require_supported(engine_name)
    ...
    default_settings = get_engine_defaults(normalized)
...
def remove_engine(config_path: Path, config: dict, engine_name: str) -> None:
...
def set_default_engine(config_path: Path, config: dict, engine_name: str) -> None:
...
def set_model_path(config_path: Path, config: dict, engine_path: str) -> None:
...
def set_defaults(config_path: Path, config: dict) -> None:
...
def set_output_dir(config_path: Path, config: dict, output_dir: str) -> None:
...
def run_interactive_wizard(
    ...
    while True:
        ...
        if choice == "1":
            wizard_configure_engines(config_path, config, available_engines)
        elif choice == "2":
            set_defaults(config_path, config)
        elif choice == "3":
            current_dir = config.get("output_dir", ".")
            new_dir = click.prompt("Output directory", default=current_dir)
            set_output_dir(config_path, config, new_dir)
        elif choice == "4":
            list_engines(config)
            click.echo("")
    ...
    click.echo("\nSetup complete. Use 'image setup show' to review settings.")


def wizard_configure_engines(
    config_path: Path,
    config: dict,
    ...
    if 1 <= choice <= len(available_engines):
        ...
        add_engine(config_path, config, engine_name)
