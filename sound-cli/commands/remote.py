...
def run_remote_charisma(input_path: Path) -> dict:
    ...
    with app.run():
        result = spawn_and_get(
            remote_charisma, input_path.read_bytes(), input_path.name
        )
    return result
...
def run_remote_normalize_volume_measure(input_path: Path) -> dict:
    ...
    with app.run():
        result = spawn_and_get(
            remote_normalize_volume_measure, input_path.read_bytes(), input_path.name
        )
    return result
...
def run_remote_normalize_volume_apply(input_path: Path, output_path: Path, target_db: float) -> dict:
    ...
    with app.run():
        result = spawn_and_get(
            remote_normalize_volume_apply, input_path.read_bytes(), input_path.name, target_db
        )
